...
public abstract class ChessMoveAbs {
    ...
    public void moveOperate(MoveNode moveNode) {
        ...
        chessParam.maskBoardCheeses.assignOr(MaskChesses[destSite]);
        //修改着子方位棋盘
        chessParam.bitBoards[srcPlay].assignXor(MaskChesses[srcSite]);
        chessParam.bitBoards[srcPlay].assignXor(MaskChesses[destSite]);
        //修改此角色的位棋盘
        ...
        if (destChess != NOTHING) {
            ...
            //修改被吃方的位棋盘
            chessParam.bitBoards[destPlay].assignXor(MaskChesses[destSite]);
            //修改被吃方的角色位棋盘
    ...
    public void unMoveOperate(MoveNode moveNode) {
        ...
        chessParam.baseScore[destPlay] += evaluateCompute.chessAttachScore(destChessRole, destSite);
        //改全局变位棋盘(将目标位置还原)
        chessParam.maskBoardCheeses.assignXor(MaskChesses[destSite]);
        //修改着子方位棋盘
        chessParam.bitBoards[destPlay].assignXor(MaskChesses[srcSite]);
        chessParam.bitBoards[destPlay].assignXor(MaskChesses[destSite]);
        //修改着子方角色位棋盘
    ...
    public void chessEatMove(int chessRole, int srcSite, int play) {
        ...
        switch (chessRole) {
            ...
            case BLACKKING:
                //将吃子着法
                bitBoard = BitBoard.assignAndToNew(KingBitBoard[srcSite], chessParam.bitBoards[1 - play]);
                break;
            case REDGUARD:
            case BLACKGUARD:
                bitBoard = BitBoard.assignAndToNew(GuardBitBoard[srcSite], chessParam.bitBoards[1 - play]);
                break;
            case REDSOLDIER:
            case BLACKSOLDIER:
                bitBoard = BitBoard.assignAndToNew(SoldiersBitBoard[play][srcSite], chessParam.bitBoards[1 - play]);
                break;
            default:
                System.out.println("没有这个棋子:" + srcSite);
        }
    ...
    public void chessNopMove(int chessRole, int srcSite, int play) {
        ...
        switch (chessRole) {
            ...
            case BLACKKNIGHT:
                ...
                //走到的位置有棋子的
                bitBoard = BitBoard.assignAndToNew(attackBoard, chessParam.maskBoardCheeses);
                //拿出走到的位置没有棋子的
            ...
            case BLACKKING:
                //将着到的位置有棋子的
                bitBoard = BitBoard.assignAndToNew(KingBitBoard[srcSite], chessParam.maskBoardCheeses);
                //将着到的位置没有棋子的
            ...
            case BLACKGUARD:
                //走的位置有棋子
                bitBoard = BitBoard.assignAndToNew(GuardBitBoard[srcSite], chessParam.maskBoardCheeses);
                //取出没有棋子的
            ...
            case BLACKSOLDIER:
                //有棋子
                bitBoard = BitBoard.assignAndToNew(SoldiersBitBoard[play][srcSite], chessParam.maskBoardCheeses);
                //取出无棋子
