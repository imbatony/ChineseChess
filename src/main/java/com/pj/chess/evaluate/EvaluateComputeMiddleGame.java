...
public class EvaluateComputeMiddleGame extends EvaluateCompute {
    ...
    public int evaluate(int play) {
        ...
        for (int i = 0; i < 2; i++) {
            ...
            //对手棋子数量
            int oppAllChessNum = chessParam.getAttackChessesNum(oppPlayTemp) + chessParam.getDefenseChessesNum(
                oppPlayTemp) - 1;
            ...
            //将不在中线
            if (boardCol[oppKingSite] == 3) {
                defensePartition[oppPlayTemp][LEFTSITE] -= 1;

            } else if (boardCol[oppKingSite] == 5) {
                defensePartition[oppPlayTemp][RIGHTSITE] -= 1;

            } else if (boardCol[oppKingSite] == 4) {//将不在底线
